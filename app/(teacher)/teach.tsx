...
export default function Classes(props: Props) {
  ...
  const clickClass=(sub:Subject)=>{
    router.push({
      pathname: '/(teacherDetail)/classDetail/feature', 
      params: {
        subjectId:sub.id,
        name:sub.name,
        code:sub.code
  ...
  const checkTime = (times: Time[]) => {
    ...
    for (const time of times) {
      ...
      if (time.numberOfWeek === currentDay && 
        ...
        {listSub.map((item: Subject, index: number) => (
          <TouchableOpacity key={index} onPress={()=>clickClass(item)}>
            ...
            {checkTime(item.times)?(
              <Text className='text-green font-medium ml-[2px] mt-[-4px] mb-2'>Đang diễn ra</Text>
            ):('' )}  
            <View className='flex-row items-center ml-[3px]'>
              <FontAwesome5 name="book" size={22} color="black" />
              <Text className='text-base ml-4 font-msemibold'>{item.code} </Text>
            </View>
           
            <View className='flex-row items-center mt-[4px] ml-[1px]'>
            <MaterialCommunityIcons name="checkbook" size={24} className='w-5 h-7' color="black" />
              <Text numberOfLines={2} ellipsizeMode='tail' className='w-[85%] text-base ml-[14px] font-mregular'>{item.name}</Text>
            </View>
