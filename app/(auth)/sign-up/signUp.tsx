...
import { SafeAreaView, View, Text, ScrollView, TextInput, TouchableOpacity, Image, Alert } from 'react-native'
...
export default function SignUp({ }: Props) {
  ...
  const register = async () => {
    ...
    {
      Alert.alert('Thông báo','Vui lòng nhập đầy đủ email và mật khẩu để đăng kí')
      return;
    }
    if (pass != confirm) {
  ...
  return (
    <SafeAreaView>
      ...
      <ScrollView>
        <View>
          <Text className="text-blue_primary text-[28px] font-msemibold mt-[30%] ml-[8%]">Tạo tài khoản</Text>
          ...
          {/* Xác nhận Mật khẩu */}
          <View className={` ml-auto mr-auto w-[85%] mt-4 pb-1`}>
            <Text className="text-[16px] font-msemibold text-gray_primary mb-2">Confirm Password</Text>
            <View className="border-[1.2px] rounded-2xl border-gray-400 px-2 py-[6px] flex-row justify-center items-center">
              <TextInput className="text-[14px] font-mmedium text-gray-800 flex-1"
                secureTextEntry={visible}
                autoCorrect={false}
          ...
          <View className="flex-row w-[85%] mx-auto items-center mt-[3%] pl-1">
            <TouchableOpacity onPress={() => setCheck(!check)}>
              <FontAwesome name={check ? "check-square-o" : "square-o"} size={22} color={colors.text_gray} />
            </TouchableOpacity>
          ...
          <View>
            <View className="flex-row items-center w-[85%] mt-[10%] ml-auto mr-auto">
              ...
              <Text className="text-slate-500 mx-1 font-msemibold text-xs" >hoặc đăng kí với</Text>
              <View className="bg-slate-500 flex-1 h-[1px]"></View>
            </View>
