...
export default function ReSchedule({}: Props) {
  ...
  const createNotification = async () => {
    ...
    setLoading(true);
    const url = `${localHost}/api/v1/subject/notify/classReschedule`;
    const data = {
      subjectId: subjectId,
      ...
      if (response.status == 200) {
        setNotification(
          (prev: any) => [...prev, {
            ...
            content: `Thông báo lịch học vào ngày ${formatDate(date.toString())}.`,
          }]
        );
        Alert.alert("Thông báo", "Đã gửi thông báo học bù thành công");
      } else {
  ...
  const getData = async () => {
    const url = `${localHost}/api/v1/notification/classReschedule/${subjectId}`;
      ...
      <Modal
        ...
        <TouchableOpacity
          ...
          <View className="bg-white w-[90%] rounded-2xl px-4 py-4">
              <Text className="text-center font-bold text-xl">Thông báo học bù</Text>
              <Text className="text-center">{code}</Text>
              <Text className="">Ngày bù</Text>
              <TouchableOpacity
                onPress={chooseDate}>
                <Text
