    ...
    for (let i = start; i < totalMessages; i++) {
      ...
      if(!sender)
      {
          ...
          if (formatName) {
            ...
            listFormat.push({ id: currentPost.creator.id, number: numberName + 1 });
            setNumberName(numberName + 1);
            nameAnonymous = `Ẩn danh ${num}`;
          }
      }
      else{
      ...
      if (i > 0) {
        ...
        if (previousPostTime.getDate() !== time.getDate()) {
          list.push(
            <Text key={formatDateTime(currentPost.createdAt)} className="bg-[#BBB3B3] rounded-[10px] py-[3px] max-w-[200px] font-semibold text-[10px] mt-5 mx-auto text-white px-[7px]">
  ...
  const handleAddDocument = async () => {
    if (selectedImages.length > 0) {
      ...
      try {
        ...
        const uploadPromises = selectedImages.map((item) =>
          uploadImage(item)
        );
        ...
        <View className="flex-1 justify-center items-center z-50 relative" style={{ backgroundColor: 'rgba(170, 170, 170, 0.8)' }}>
          ...
          <View className='bg-white px-[8%] w-[90%]  pt-3 pb-4 rounded-lg shadow-lg'>
            ...
            <View className=' mt-3 py-2 px-3  border border-gray-300 rounded-md'>
              <TextInput placeholder='Thắc mắc...' value={titlePost} onChangeText={(e) => setTitlePost(e)} className='text-base leading-[22px]' />
            </View>
