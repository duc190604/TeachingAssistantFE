...
import { View, Text, ScrollView, TouchableOpacity, Image,  TextInput, Modal, Alert, Pressable } from 'react-native';
...
export const Question = ({Content,User,Avatar,Time,Type,SenderId,Id,IsRecall,HandleRecall}:props) => {
    ...
    const reCall = async () => {
    const res = await patch({url: `${localHost}/api/v1/question/update/${Id}`, data: {isResolved: true}, token: accessToken})
    if(res){
      if(res.status == 200){
        HandleRecall(Id)
    ...
    if (User == "My message") {
                ...
                case 'image':
                    contentType =
                        <View style={{ marginTop: 3, marginRight: 5 }}>
                            <Pressable className="w-[150px] h-[200px] overflow-hidden rounded-xl border border-blue_primary" onPress={openModal} onLongPress={()=>setModalRecallVisible(true)} >
        ...
        return (
            <Pressable className="mr-[2%]" onLongPress={()=>setModalRecallVisible(true)} >
                {modalRecall()}
                {modalImage()}
